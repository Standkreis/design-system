...
- `npm run test:browser` passed all ten tests across desktop and emulated mobile viewports. Scenarios cover logo variants, EN/DE labels, identification-dialog focus and save feedback, sheet keyboard behaviour, reduced motion, system/manual theme handling, preference persistence, form controls and application tabs.
...
- Logo invariants cover equal arcs/gaps, the right-facing opening, optional centred dot and matching downloadable SVGs. Sixteen semantic reading/action colour pairs meet a 4.5:1 threshold.
